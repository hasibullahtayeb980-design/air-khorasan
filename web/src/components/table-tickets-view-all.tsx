...
import teamMembers from "@/components/application/table/tickets.json";
    ...
    return (
        <TableCard.Root>
            ...
            <Table aria-label="Team members" selectionMode="multiple" sortDescriptor={sortDescriptor} onSortChange={setSortDescriptor}>
                <Table.Header>
                    <Table.Head id="name" label="Ticket ID" isRowHeader allowsSorting className="w-full max-w-1/4" />
                    <Table.Head id="status" label="Customer" allowsSorting />
                    <Table.Head id="email" label="Phone" allowsSorting className="md:hidden xl:table-cell" />
                    <Table.Head id="role" label="Airline" allowsSorting tooltip="This is a tooltip" />
                ...
                <Table.Body items={sortedItems}>
                    {(item) => (
                        <Table.Row id={item.username}>
                            ...
                            <Table.Cell className="whitespace-nowrap md:hidden xl:table-cell">{item.phone}</Table.Cell>
                            <Table.Cell className="whitespace-nowrap">{item.airline}</Table.Cell>
                            <Table.Cell className="whitespace-nowrap">{item.from_city}</Table.Cell>
                            <Table.Cell className="whitespace-nowrap">{item.to_city}</Table.Cell>
                            <Table.Cell className="whitespace-nowrap">{item.departure_date}</Table.Cell>
